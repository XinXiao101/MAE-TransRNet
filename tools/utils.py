...
def HD(x,y):
    ...
    hd_set = []
    for i in range(4):
        hd_set.append(hd(x[:,i,...],y[:,i,...]))
    hd_set.append(hd(x,y))
    return hd_set
# from hausdorff import hausdorff_distance
# def HD(x,y,mode='euclidean'):
#     x = x.detach().cpu().numpy()
#     y = y.detach().cpu().numpy()
#     return hausdorff_distance(x,y, distance=mode)
...
def dice_val(y_pred, y_true):
    ...
    # print("y_pred.shape_1:",y_pred.shape)
    # # y_pred = torch.squeeze(y_pred, 1)
    # print("y_pred.shape_2:", y_pred.shape)
    # # y_pred = y_pred.permute(0, 4, 1, 2, 3).contiguous()
    # print("y_pred.shape_3:", y_pred.shape)
